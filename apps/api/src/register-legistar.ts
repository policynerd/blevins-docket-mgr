...
export function registerLegistar(
  ...
  app.patch('/files/:id', async (req) => {
    ...
    const body = z.object({ sponsors: z.string().max(2000).optional(), agendaDate: z.string().nullable().optional() }).parse(req.body);
  ...
  app.post('/files/:id/actions', async (req) => {
    ...
    const body = z
      ...
      .parse(req.body);
    const recorded = await recordAction(db, { proposalId: id, ...body, userId: user.id });
    ...
    void pushToDocket({
      ...
      title: file.title,
      status: file.status,
      inControl: file.inControl,
      agendaDate: file.agendaDate ? String(file.agendaDate) : null,
      enactmentNumber: file.enactmentNumber,
      sponsors: file.sponsors,
      packetUrl: `/proposals/${file.id}/print`,
    });
    return recorded;
  });
  app.post('/actions/:id/certify', async (req) => {
    const user = await requireUser(req);
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const { note } = z.object({ note: z.string().max(2000).optional() }).parse(req.body ?? {});
    return certifyAction(db, id, user.id, note);
  });
  ...
  app.post('/meetings/:id/generate', async (req) => {
    ...
    return generateAgenda(db, id, user.id, reason);
  });
  ...
  app.post('/meetings/:id/publish', async (req) => {
    ...
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const { reason } = z.object({ reason: z.string().max(2000).optional() }).parse(req.body ?? {});
    await publishAgenda(db, id, user.id, reason);
    return getMeeting(db, id);
  });
  app.post('/meetings/:id/amend', async (req) => {
    const user = await requireUser(req);
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const { reason } = z.object({ reason: z.string().min(1).max(2000) }).parse(req.body);
    return createAgendaAmendment(db, id, user.id, reason);
  });
  app.post('/meetings/:id/events', async (req) => {
    const user = await requireUser(req);
    const { id } = z.object({ id: z.string().uuid() }).parse(req.params);
    const body = z
      .object({
        eventType: z.enum(['MEETING_CALLED_TO_ORDER', 'ROLL_CALL', 'MOTION', 'MEETING_ADJOURNED', 'MINUTES_ADOPTED']),
  ...
  app.get('/publications', async (req) => {
    const q = z.object({ meetingId: z.string().uuid().optional(), proposalId: z.string().uuid().optional() }).parse(req.query);
    return listPublications(db, q);
  });
