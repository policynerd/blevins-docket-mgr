    ...
    <>
      ...
      <header className="record-hero">
        <div>
          ...
          <div className="record-tags">
            ...
            <em className={`status-chip status-${status.toLowerCase().replaceAll(' ', '-')}`}>
              {status}
            </em>
          </div>
        </div>
        <div className="record-actions">
          <a className="btn" href={`/proposals/${id}/print`}>
            Official copy
          </a>
          <a className="btn" href={`/api/proposals/${id}/export.pdf`}>
            Download PDF
          </a>
          <a
            className="btn primary"
            href={proposal.documents[0] ? `/documents/${proposal.documents[0].id}?proposal=${id}` : '#'}
          >
      ...
      <div className="record-facts">
        ...
        <div>
          ...
          <strong>{file?.agendaDate ? new Date(file.agendaDate).toLocaleDateString() : '—'}</strong>
        </div>
        <div>
          <span>Sponsors</span>
          <strong>{file?.sponsors || '—'}</strong>
        </div>
        <div>
          <span>Final Action</span>
          <strong>{file?.finalActionAt ? new Date(file.finalActionAt).toLocaleDateString() : '—'}</strong>
        </div>
        <div>
          <span>Enactment</span>
          <strong>{file?.enactmentNumber || '—'}</strong>
        </div>
      </div>
        ...
        <div className="dashboard-grid">
                ...
                history.slice(0, 8).map((h) => (
                  <div className="timeline-row" key={h.id}>
                    ...
                    <div>
                      ...
                      {h.result ? <small>Result: {h.result}</small> : null}
                    </div>
                  </div>
                ))
              ) : (
          ...
          <section>
            ...
            <div className="card detail-list">
              ...
              <div>
                <span>Documents</span>
                <b>{proposal.documents.length}</b>
              </div>
              <div>
                <span>Official snapshots</span>
        ...
        <div className="card">
          {proposal.documents.map((d) => (
            <a className="row" key={d.id} href={`/documents/${d.id}?proposal=${id}`}>
              ...
              <div className="meta">
                {d.docType} · {d.version?.label ?? 'No version'}
              </div>
              <div className="hint">Open document</div>
            </a>
          ))}
        </div>
      ) : null}
            ...
            history.map((h) => (
              <div className="timeline-row" key={h.id}>
                ...
                <div>
                  <strong>{h.action}</strong>
                  <p>{h.actionText || h.actionNote || h.actingBody}</p>
                  {h.sentTo ? <small>Sent to {h.sentTo}</small> : null}
                </div>
              </div>
            ))
          ) : (
          ...
          {history.length ? (
            history.map((h) => (
              <div className="row" key={h.id}>
                <div className="title">{h.action}</div>
                <div className="meta">
                  {h.actingBody} · {new Date(h.actionAt).toLocaleString()}
                  {h.result ? ` · ${h.result}` : ''}
                </div>
                {h.votes.length ? (
                  <div className="vote-line">
                    {h.votes.map((v) => (
                      <span key={v.memberName}>
                        {v.memberName}: <b>{v.vote}</b>
                      </span>
                    ))}
                  </div>
                ) : null}
            ...
            <div className="empty">No actions recorded.</div>
          )}
        </div>
      ) : null}
      {tab === 'versions' ? (
          ...
          {milestones.length ? (
            milestones.map((m) => (
              <div className="row" key={m.id}>
                ...
                <div className="meta">Frozen record · {new Date(m.createdAt).toLocaleString()}</div>
              </div>
            ))
          ) : (
      ...
      {tab === 'audit' ? (
        <div className="card">
          <div className="empty">
            Entity-level audit will appear here as the publication and audit ledger is wired into
            the record model.
          </div>
        </div>
      ) : null}
